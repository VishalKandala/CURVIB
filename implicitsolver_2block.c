...
PetscErrorCode  Implicit_SNES(UserCtx *user, IBMNodes *ibm, FSInfo *fsi)  //Solver No.5
{
  ...
  for (bi=0; bi<block_number; bi++) {
    ...
    //  SNESSetLagJacobian(user[bi].snes,-2);
    //End--------------------Finite difference jacobian--------------------------------
    } else if(implicit_type==2){
    //Start--------------------Matrix Free---------------------------------------------
      DMCreateMatrix(user[bi].fda,&user[bi].PJ);
        MatCreateSNESMF(user[bi].snes, &user[bi].J);
	SNESSetJacobian(user[bi].snes,user[bi].J,user[bi].J,MatMFFDComputeJacobian,(void *)&user[bi]);
	//    SNESSetJacobian(user[bi].snes,user[bi].J,user[bi].PJ,FormJacobian_MF,(void *)&user[bi]);
      ...
      VecGetSize(user[bi].Rhs,&N);
      VecGetLocalSize(user[bi].Rhs, &M);
      MatCreateAIJ(PETSC_COMM_WORLD, M, M, N, N,51, PETSC_NULL,51, PETSC_NULL,&user[bi].J);  //for diagonal 3 , 51 for jacobian
   ...
   //-------------- End Null space removing
    ...
    if(implicit_type==2){
      ...
      FormJacobian_Diagonal_MF(user[bi].PJ,(void *)&user[bi]);
      SampleShellPCSetUp(pc,user[bi].PJ,user[bi].Ucont);
...
//----Start visualization-------------------------------------------------------
  PetscInt visualize=0; 
  if(visualize==1){
 
  Cmpnts ***UU;
  PetscInt i,k;
  bi=0;
  PetscReal Sumy=0.0,Sum=0.0;
  DMDAVecGetArray(user[bi].fda,user[bi].Ucont, &UU);
  
   for (k=user[bi].info.zs; k<(user[bi].info.zs+user[bi].info.zm); k++) {
     //  for (j=user[bi].info.ys; j<(user[bi].info.ys+user[bi].info.ym); j++) {
         for (i=user[bi].info.xs; i<(user[bi].info.xs+user[bi].info.xm); i++) {
      Sumy=UU[k][3][i].y-UU[k][0][i].y;
      PetscPrintf(PETSC_COMM_WORLD, "Sum[%d][%d]:{%le=%le-%le} \n",k,i,Sumy,UU[k][3][i].y,UU[k][0][i].y);
      Sum+=Sumy;
    }
  }
  //   }
  PetscPrintf(PETSC_COMM_WORLD, "-----------------------------------\n",k);
  PetscPrintf(PETSC_COMM_WORLD, "Sum:%le \n",Sum);
  DMDAVecRestoreArray(user[bi].fda,user[bi].Ucont, &UU);
  }
...
//Recover user context
 ...
 //----Compute RHS as usual
  ...
  VecSet(R,0.);
  DMCompositeGetAccess(usermg->packer, R,&Rb[0],&Rb[1]);
  for (bi=0; bi<block_number; bi++) {
    VecCopy(user[bi].Rhs, Rb[bi]);
  }
  DMCompositeRestoreAccess(usermg->packer, R,&Rb[0],&Rb[1]);
