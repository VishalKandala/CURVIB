...
PetscErrorCode spline (PetscInt n, PetscInt end1, PetscInt end2, 
		       PetscReal slope1, PetscReal slope2,
		       PetscReal x[], PetscReal y[], PetscReal b[], 
		       PetscReal c[], PetscReal d[], PetscInt    *iflag)

/* PetscInt     n, end1, end2; */
/* PetscReal    slope1, slope2; */
/* PetscReal    x[], y[], b[], c[], d[]; */
/* PetscInt    *iflag; */
...
PetscReal   t;
PetscInt    ascend;
//PetscPrintf(PETSC_COMM_WORLD, "Init S %d\n",n);
...
//PetscPrintf(PETSC_COMM_WORLD, "Init S\n");
ascend = 1;
/* for (i = 1; i < n; ++i) { */
/*   //PetscPrintf(PETSC_COMM_WORLD, "Init %d %le %le \n",i,x[i],x[i-1]); */
...
//  PetscPrintf(PETSC_COMM_WORLD, "Init S>3\n");
if (n >= 3)
   {    /* ---- At least quadratic ---- */
...
//PetscPrintf(PETSC_COMM_WORLD, "sVal %d %le %le %le %le %le %le \n",i,u,w,y[i],b[i],c[i],d[i]);

return(w);
}

double Sval (int n, double u, double x[], double y[], double b[], 
	     double c[], double d[], int *last)
